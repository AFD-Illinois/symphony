...
double kappa_Q_abs(struct parameters * params)
{
  ...
  double hyp2f1 = pow(1.-z, -a)*tgamma(c)*tgamma(b-a)/(tgamma(b)*tgamma(c-a))
                 ...
                 *tgamma(c)*tgamma(a-b)/(tgamma(a)*tgamma(c-b))
                 *gsl_sf_hyperg_2F1(b, c-a, b-a+1., 1./(1.-z));
  double Nlow = -(25./48.)*pow(3., 1./6.)*(10./41.)*pow(2.*params->pi, 2.)
               /pow(params->kappa_width*params->kappa, 16./3.-params->kappa)*(params->kappa-2.)*(params->kappa-1.)
               *params->kappa/(3.*params->kappa-1.)*tgamma(5./3.)*hyp2f1;
  double Nhigh = -(pow(21., 2.)*pow(params->kappa, -144./25.)+11./20.)*2.
                 *pow(params->pi, 5./2.)/3.*(params->kappa-2.)*(params->kappa-1.)*params->kappa
                 /pow(params->kappa_width*params->kappa, 5.)*(2*tgamma(2.+params->kappa/2.)
                 /(2.+params->kappa)-1.);
...
double kappa_V_abs(struct parameters * params)
{
  ...
  double hyp2f1 = pow(1.-z, -a)*tgamma(c)*tgamma(b-a)/(tgamma(b)*tgamma(c-a))
                 ...
                 *tgamma(c)*tgamma(a-b)/(tgamma(a)*tgamma(c-b))
                 *gsl_sf_hyperg_2F1(b, c-a, b-a+1., 1./(1.-z));
  double Nlow = -(77./(100.*params->kappa_width))*pow(pow(sin(params->observer_angle), -114./50.)
                -1., 223./500.)*pow(X_k, -7./20.)*pow(params->kappa, -7./10)
                *pow(3., 1./6.)*(10./41.)*pow(2.*params->pi, 2.)
                /pow(params->kappa_width*params->kappa, 16./3.-params->kappa)*(params->kappa-2.)*(params->kappa-1.)
                *params->kappa/(3.*params->kappa-1.)*tgamma(5./3.)*hyp2f1;
  double Nhigh = -(143./10. * pow(params->kappa_width, -116./125.))
                 *pow(pow(sin(params->observer_angle), -41./20.)-1., 1./2.)
                 *(13.*13.*pow(params->kappa, -8.)+13./(2500.)*params->kappa - 263./5000.+47.
                 /(200.*params->kappa))*pow(X_k, -1./2.)*2.*pow(params->pi, 5./2.)/3.
                 *(params->kappa-2.)*(params->kappa-1.)*params->kappa/pow(params->kappa_width*params->kappa, 5.)
